...
function compileSass(onCompilationComplete) {
    ...
    async.eachSeries(project.tasks.sass, function each(taskMeta, done) {
        ...
        if (taskMeta.options.useBourbon) includes.push(require('bourbon').includePaths);
        if (taskMeta.options.useCompassMixins) includes.push(path.join(taskMeta.input, 'external/compass-mixins'));
        SassCache.clear();
...
function debugSass(onCompilationComplete) {
    ...
    async.eachSeries(project.tasks.sass, function each(taskMeta, done) {
        libsass.render({
            ...
            if (done) done();
        }); }, function complete() { onCompilationComplete.apply(null, arguments); }) } module.exports = {
    debug: debugSass,
    compile: compileSass,
