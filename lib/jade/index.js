...
function compileJade(onCompilationComplete, options) {
    var _options = _.extend({fileExtension: 'php', taskName: 'jade-php'}, options),
...
function compileJadePHPDebug(onCompilationComplete) {
    async.each(project.tasks['jade-php'], function each(taskMeta, done) {
        gulp.src(path.join(taskMeta.input, '/**/[^_]*.jade'))
            .pipe(through2.obj({
                    allowHalfOpen: false
                },
...
module.exports = {
    ...
    php: function (callback) {
        compileJade(callback, {fileExtension: 'php', taskName: 'jade-php'})
    },
