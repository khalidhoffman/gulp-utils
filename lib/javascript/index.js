...
function buildRequireJSConfig(done) {
    var defaultCSSPath = path.join(wordpress.theme.path, 'stylesheets/'),
        tasks = project.tasks['boilerplate-config'] || project.tasks['js-config'];

    async.each(tasks, function each(taskMeta, onConfigComplete) {
...
function buildWebpackProjects(done){
    ...
    var tasks = project.tasks['webpack'] || project.tasks['js-webpack'];

    async.each(tasks, function each(taskMeta, onBuildComplete) {
...
function buildRjsProjects(done){
    ...
    var tasks = project.tasks['requirejs'] || project.tasks['js-bundle'];
...
function bundleRequireJS(done, options) {
    var _options = _.extend({
            buildPath: path.join(project.config.paths.workingDir, 'js/src/build.js')
        }, options),
...
function bundleWebpack(done, options) {
    var _options = _.extend({
            buildPath: path.join(project.config.paths.workingDir, 'js/src/webpack.config.js')
        }, options),
