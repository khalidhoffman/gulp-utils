...
function compile(onCompilationComplete, options) {
    var isCallbackProvided = _.isFunction(onCompilationComplete),
        _options = _.defaults(isCallbackProvided ? options : onCompilationComplete, {
            tasks : project.tasks['jsx'],
    ...
    async.each(tasks, function each(taskMeta, done){
        ...
        if (isCallbackProvided) onCompilationComplete()
    })
