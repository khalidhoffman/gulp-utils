...
function initProject(onInitializationComplete, options) {
    ...
    fs.readFile(path.resolve(config.paths.rootDir, '.gitmodules'), {encoding: 'utf8'}, function (err, content) {
        ...
        prompt.get({
            ...
            if (err){
                ...
                if (/^\s*(yes|y)\s*$/.test(result.isDPTheme)){
                    _options.originalThemeName = config.wordpress.wpThemePrefix+_options.originalProjectName;
                } else{
    ...
    function createNewTheme() {
        ...
        console.log('found git submodules: %s', projectUtils.dump(submodules));
        fsExt.copy(
    ...
    function updateGitSubmodules() {
        async.eachSeries(submodules, function each(submodulePath, done) {
            var gitProcess = require('child_process').exec,
                newSubmodulePath = submodulePath.replace(_options.originalThemeName, path.parse(config.paths.workingDir).base),
