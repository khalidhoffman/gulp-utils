        ...
        prompt.get({
            ...
            if (err){
                ...
                if (/^\s*(yes|y)\s*$/.test(result.isDPTheme)){
                    _options.originalThemeName = config.wpThemePrefix+_options.originalProjectName;
                } else{
    ...
    function updateGitSubmodules() {
        async.eachSeries(submodules, function each(submodulePath, done) {
            var gitProcess = require('child_process').exec,
                newSubmodulePath = submodulePath.replace(_options.originalThemeName, path.parse(config.getWordPressBasePath()).base),
