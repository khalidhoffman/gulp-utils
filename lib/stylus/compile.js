...
class StylusTask extends Task {
    ...
    setupEnvironment(dir, options) {
        ...
        return fs.stat(helpersDir)
            .then(dirStat => !dirStat.isDirectory() ? Promise.reject(new Error(`'${helpersDir}' is not a directory`)) : null)
        ...
        return fs.readFile(filename, {encoding: 'utf8'})
            .then(str => {
                const stylus = Stylus(str)
                    ...
                    .import(path.join(this.input, "_auto-generated/*"));
                ...
                if (this.options.settings) {
                    Object.keys(this.options.settings).forEach(key => {
                        stylus.set(key, this.options.settings[key]);
                    })
                }
