...
function dumpObj(arg1) {
    console.log.apply(console, ['dump: %s', projectUtils.dump(arg1)]);
    return arg1;
}
...
function getStylusVal(obj) {
    ...
    if (obj.val) {
        ...
        _.forEach(obj.vals, function (prop, propName) {
            result[propName] = getStylusVal(prop);
        })
    } else if (obj.nodes && obj.nodes[0]) {
        var mainNode = obj.nodes[0];
        result = getStylusVal(mainNode);
    }
    return result;
}

function writeJSON(writePath, data) {
    var dir = path.parse(writePath.filename).dir,
        absWritePath = writePath.val.match(/^\./) ? path.join(dir, writePath.val) : writePath.val,
        jsonData = getStylusVal(data);
