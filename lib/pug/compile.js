...
const log = debug('kdev-utils:pug');

log.error = debug('kdev-utils:pug:error');
...
class PugTask extends Task {
    ...
    run() {
        ...
        const autoGenIncludeRegex = /(^|\n)[\W]*include\s+.*_auto-generated\/_-all\.pug\n/;
        ...
        return Utils.persistDir(projectHelpersDir)
            ...
            .then(files => {
                return Promise.all(files.map(filePath => {
                    const filePathMeta = path.parse(filePath);
                    const destFilename = `${filePathMeta.name.replace(/\.bem/i, '')}.${this.options.fileExtension}`;
                    const destPath = path.join(filePathMeta.dir, destFilename);
                    const pugParams = {
                        filename: this.options.filename || filePath,
                        pretty: this.options.pretty,
                        doctype: this.options.doctype || 'html',
                        plugins: [bemify(), require('pug-plugin-ng')],
                        filters: Object.assign(pug.filters, customPugFilters),
                        basedir: this.options.basedir || path.resolve('/')
                    };
                    ...
                    return fs.readFile(filePath, 'utf8')
                        .then(fileContent => {
                            ...
                            if (!autoGenIncludeRegex.test(fileContent)) {
                                ...
                                const autoGenIncludesPath = path.join(this.input, '_auto-generated/_-all.pug');
